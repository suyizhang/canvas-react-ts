...
const filePath = require('../config/compile.config').filePath;
...
const params = {};
argv.filter((name) => /^--/.test(name)).map((name) => {
    let arr = name.split('=');
...
function init() {
    ...
    if (moduleNames.length === 1 && moduleNames[0] === 'all') {
        moduleNames = Object.keys(moduleConfig).filter((v) => !!moduleConfig[v].input);
    }
    // if (moduleNames.length === 1 && moduleNames[0] === 'page') {
    //     moduleNames = Object.keys(moduleConfig).filter((v) => moduleConfig[v].isPage && !!moduleConfig[v].input);
    // }
    ...
    //     moduleNames = Object.keys(moduleConfig).filter((v) => !!moduleConfig[v].isBaseComponent);
    // }
    // //此配置不提供给第3放使用
    // if (moduleNames.length === 1 && moduleNames[0] === 'businessComponent') {
    //     moduleNames = Object.keys(moduleConfig).filter((v) => !!moduleConfig[v].isBusinessComponent);
    // }
    ...
    let arr = moduleNames.map((name) =>
        build(name).then(({ stats, name, warnings, code }) => {
            ...
            code === 200 && console.log(chalk.green(`module[${name}] Compiled successfully.\n`));
        }),
    );
    // Promise.all(arr)
    //     .then(() => {
    //         // const asstes = Object.assign({}, assetManifest, manifest);
    //         // utils.writeFile(paths.resolveApp('public/asset-manifest.json'), JSON.stringify(asstes, null, 2));
    //     })
    //     .catch((err) => {
    //         if (err && err.message) {
...
function build(name) {
    ...
    const outputText = config.output ? config.output : ` /public/${filePath}/js/${name}/${name}.js`;
    console.log(chalk.yellow(`building module[${name}] entry -> ${moduleConfig[name].input}, output -> ${outputText}`));
    console.log();
